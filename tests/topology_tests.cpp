...
namespace {
    ...
    struct P1 { static void print() { std::cout << "1"; } };
    struct P2 { static void print() { std::cout << "2"; } };
    struct P3 { static void print() { std::cout << "3"; } };
    struct P4 { static void print() { std::cout << "4"; } };
    ...
    // Use compile-time apply_vertex_types (type-level) to verify visited order matches ids().
    static constexpr bool verify_type_iteration() {
        std::array<std::size_t, CompileTimeGraphT::size()> collected {};
        std::size_t idx = 0;
        CompileTimeGraphT::apply_vertex_types(
            [&] (auto vertex_wrapper) {
    ...
    static_assert(verify_type_iteration(), "Compile-time apply_vertex_types order mismatch");
}
...
TEST_CASE("topology callable compile-time Vertex test") {
    ...
    char visited[4] {}; char* write = visited;

    Topo::apply_vertex_types([&] (auto vertex_wrapper) {
